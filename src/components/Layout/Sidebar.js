    ...
    switch (user.role) {
      case "doctor":
        return [
          ...
          { icon: Calendar, label: "My Feed", path: "/doctor/myfeed" },
  ...
  return (
    <div className="w-64 bg-gradient-to-b from-slate-50 to-white border-r border-gray-200 shadow-lg relative">
      ...
      <div className="p-6 border-b border-gray-200 bg-white relative overflow-hidden">
        <div
          className={`absolute inset-0 bg-gradient-to-r ${colors.secondary} opacity-30`}
        />
        <div className="relative flex items-center space-x-3">
          <div
            className={`w-12 h-12 bg-white rounded-xl flex items-center justify-center shadow-lg hover-lift overflow-hidden`}
          >
            <img
              src={MediLinkLogo}
              alt="MediLink Logo"
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <div className="relative flex items-center space-x-3 z-10">
                  ...
                  <span
                    className={`font-medium ${
                      isActive
                        ? "text-white"
                        : `text-gray-700 ${colors.hoverText}`
                    } transition-colors duration-300`}
                  >
                    {item.label}
                  </span>
                ...
                {isActive && (
                  <div
                    className={`absolute left-0 top-0 w-1 h-full bg-white rounded-r-full shadow-lg`}
                  />
                )}
              </>
            )}
      ...
      <div
        className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[${colors.accentLight}] to-transparent opacity-50`}
      />
    </div>
  );
};
